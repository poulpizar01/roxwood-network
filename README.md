...
- `src/services/webhookDispatcher.ts` : webhooks sortants signés HMAC (header `X-Signature-256`) sur les événements `monitoring.shift`, `monitoring.recruitment`, `monitoring.safe`, `monitoring.invoice`, `monitoring.sale`, `absence.updated`, `order.updated` — point de branchement générique pour un CRM/site externe. Tous gérés sans accès DB, exclusivement depuis le panneau "Monitoring" (`WEBHOOK_EVENT_LABELS` y liste les 7 types, quel que soit leur domaine — voir la section Monitoring). Un événement `ticket.created`/`ticket.closed` a existé un temps mais a été retiré : aucune UI panneau ne l'exposait et aucun besoin concret ne le motivait — à réintroduire si un vrai cas d'usage se présente.
